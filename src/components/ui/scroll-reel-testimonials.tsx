...
function Chars({
  ...
  return (
    <>
      {words.map((word, wi) => {
        const wordSpan = (
          <span className="inline-block whitespace-nowrap" key={`word-${wi}`}>
...
export function ScrollReelTestimonials({
  ...
  return (
    <div
      ...
      onKeyDown={onKeyDown}
      className={cn(
        "relative flex w-full max-w-[1060px] flex-col items-stretch gap-8 overflow-hidden rounded-xl border border-border bg-muted shadow-[inset_0_2px_0_rgba(255,255,255,1)] outline-none focus-visible:ring-2 focus-visible:ring-ring md:min-h-[320px] md:flex-row md:gap-12",
    ...
    >
      ...
      {/* Content section */}
      <div
        className="flex min-w-0 flex-1 flex-col justify-between self-stretch p-6 md:p-10"
      >
        <div className="flex flex-col gap-[9px]">
          <Quote className="block h-12 w-12 text-muted-foreground/40 fill-current" aria-hidden="true" />

          {/* Text stage */}
          <div
            className="relative w-full max-w-full md:max-w-[450px] overflow-hidden"
            aria-live="polite"
          >
            {/* Invisible in-flow copy sizes the stage to the largest
              * quote at any viewport width, so wrapped text never clips and size remains constant. */}
            <div
              aria-hidden="true"
              className="invisible grid min-h-[140px]"
            >
              {testimonials.map((t, idx) => (
                <div key={idx} className="col-start-1 row-start-1 flex flex-col gap-[19px]">
        ...
        {/* Controls */}
        <div className="mt-10 flex items-center gap-4 md:mt-auto">
          <button
            type="button"
            onClick={() => paginate(-1)}
            disabled={index === 0}
            aria-label="Previous testimonial"
            className="grid h-12 w-12 cursor-pointer place-items-center rounded-full border border-foreground/15 bg-transparent p-2 m-1 text-foreground transition-[opacity,transform] duration-200 ease-[cubic-bezier(0.22,1,0.36,1)] hover:enabled:scale-[1.08] active:enabled:scale-[0.94] disabled:cursor-default disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
          >
